...
mask =  'MNI_bothCons_thrAbs0p5_X_322_both_07_left_phc_1p5';
decodingDataDir = fullfile(root,'decoding','data',mask);

corr_irrelScore_dIrrel = nan(length(subs),2);
nLambda = 20;
lambda = logspace(-5,5,nLambda); % from 10^-5 to 10^5
...
disp(sprintf('mask: %s, max accuracy session 1: %.2f, with %s penalty, lambda = %.5f ',mask,M,strRegulType{indRegulType},lambda(indLambda)))

meanAccSess2 = squeeze(mean(mean(accuracy(:,2,:,:,:),1),3)); % iRegulType x nLambda
[M, I_lin] = max(meanAccSess2(:));
[indRegulType, indLambda] = ind2sub([3,nLambda],I_lin);
disp(sprintf('mask: %s, max accuracy session 2: %.2f, with %s penalty, lambda = %.5f ',mask,M,strRegulType{indRegulType},lambda(indLambda)))


meanAccBothSess = squeeze(mean(mean(mean(accuracy,1),2),3)); % iRegulType x nLambda
[M, I_lin] = max(meanAccBothSess(:));
[indRegulType, indLambda] = ind2sub([3,nLambda],I_lin);
disp(sprintf('mask: %s, max accuracy both sessions: %.2f, with %s penalty, lambda = %.5f ',mask,M,strRegulType{indRegulType},lambda(indLambda)))
