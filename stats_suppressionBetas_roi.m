%% =========================================================
%  ROI beta tests: 4 cells (session x graph)
...
root = ['/vols/Scratch/mgarvert/ManyMaps/imagingData/2ndLevel/design_322_fsl_/mask/' mask];
...
for iSub = 1:nSub
    for iSess = 1:nSess
        try
            rel_file = fullfile(root, rel_con, ...
                [sprintf('%02d', iSub) '_' num2str(iSess) ...
                '_' mask '_' rel_con '.txt']);

            irrel_file = fullfile(root, irrel_con, ...
                [sprintf('%02d', iSub) '_' num2str(iSess) ...
                '_' mask '_' irrel_con '.txt']);

            betas(iSub, iSess, 1) = load(rel_file);
            betas(iSub, iSess, 2) = load(irrel_file);
        catch
        end
    end
end
...
% Holm: sort, multiply by descending ranks, enforce monotonicity, map back
...
% Put the 4 cells into a wide table, one row per subject
Y = table( ...
    betas(:,1,1), betas(:,1,2), betas(:,2,1), betas(:,2,2), ...
    'VariableNames', {'S1_Rel','S1_Irrel','S2_Rel','S2_Irrel'} );

% Within-subject design table must match the order of the response columns
within = table( ...
...
disp(' ');
disp('=== Cell means (subjects averaged) ===');
