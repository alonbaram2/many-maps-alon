function stats_roi(rootData, subjects, sessions, roi, model)
...
%% Test for effect of relevant and irrelevant distances - Figure S2 G-H
if (strcmp(model,'relIrrel_sameMap') | strcmp(model,'relIrrel_sameMapNoSameCond'))
    ...
    for iSub = 1:length(subjects)
        for iSess = 1:length(sessions)
            ...
            rdmMatFile = fullfile(rdmDir,sprintf('%s_ROI_RDM_correlation_collapsed.mat', roi));            
    ...
    %% plotting the betas (plots in paper were created with Python, this code is here for completion). 
    ...
    %% Run ANOVA on the betas (for the paper was run in Python, this code is here for completion)
