...
switch analysisName
    ...
    case 'distRel_sameMapNoSameCond'
        RDM = [data.map{1,1} NaN(17); NaN(17) data.map{2,2}]; 
        RDM(logical(eye(length(RDM)))) = nan(nElem,1);
    ...
    case 'distIrrel_sameMapNoSameCond'
        RDM  = [data.map{1,2} NaN(17); NaN(17) data.map{2,1}];  
        RDM(logical(eye(length(RDM)))) = nan(nElem,1);
...
if plotFlag
    ...
    subplot(1,2,1)
    if strcmp(analysisName,'distRel_sameMapNoSameCond') || strcmp(analysisName,'distRel_sameMap') || ...
            strcmp(analysisName,'distIrrel_sameMapNoSameCond') || strcmp(analysisName,'distIrrel_sameMap')        imagescwithnan(RDM,hot,[0 1 1],true,[0,6])        
    else
    ...
    set(gca, 'FontSize', 14);    
    axis square
    ax.XTick = [1,9,18,26,34];
    ax.YTick = [1,9,18,26,34];

    subplot(1,2,2)
    RDM_noDiag=RDM;
    RDM_noDiag(logical(eye(length(RDM)))) = nan;
    if strcmp(analysisName,'distRel_sameMapNoSameCond') || strcmp(analysisName,'distRel_sameMap') || ...
            strcmp(analysisName,'distIrrel_sameMapNoSameCond') || strcmp(analysisName,'distIrrel_sameMap')
        imagescwithnan(RDM_noDiag,hot,[.7 .7 .7],true,[0,6])
    else
        imagescwithnan(RDM,hot,[.7 .7 .7])
    end
    title('withinRun')
    ax = gca();
    set(gca, 'FontSize', 14);
