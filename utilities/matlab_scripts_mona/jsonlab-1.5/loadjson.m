...
function object = parse_array(inStr, esc, varargin) % JSON array is written in row-major order
	...
	if(jsonopt('FastArrayParser',1,varargin{:})>=1 && arraydepth>=jsonopt('FastArrayParser',1,varargin{:}))
            ...
            if(~isempty(e1l) && ~isempty(e1r)) % the array is in 2D or higher D
                ...
                %%%%% ADDED BY MONA - START 
                positionOfElementsL = strfind(arraystr,'[');
                positionOfElementsR = strfind(arraystr,']');
                
                for ix = 1:length(positionOfElementsL)-1
                    dim2(ix)=length(str2num(arraystr(positionOfElementsL(ix+1):positionOfElementsR(ix))));
                end    
                %%%%% ADDED BY MONA - END
            ...
            try
                if(~isempty(dim2))
                    ...
                    if(nextidx>=length(astr)-1 && length(unique(dim2))==1)
                        object=reshape(obj,dim2(1),numel(obj)/dim2(1))';
                        pos=endpos;
                        parse_char(inStr, ']');
                        if(pbar>0)
