function projectToSurfAndSmooth(rootData,outputDir,fname,subj,fwhm)
% note we have already changed directory to outputDir before calling this
% function. 

hemName = {'LeftHem','RightHem'};
hem     = {'lh','rh'};
caretDir = fullfile(rootData,'rsa_alon',subj,'caretDirs');
[~, faces] = read_surf(fullfile(rootData,'FS','fsaverage','surf','lh.sphere.reg'));

for h=1:2
    % the surfaces in caretSDir are already in world (not
    % RAS) coordinates
    caretSDir = fullfile(caretDir,sprintf(['x',subj]),hemName{h});
    ...
    system([getenv('FREESURFER_HOME') 'bin/'...   
