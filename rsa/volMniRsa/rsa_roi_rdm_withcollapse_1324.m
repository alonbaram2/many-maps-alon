function rsa_roi_rdm_withcollapse_1324(root, subj, roi, distType, session, outDirPerROI)
% rsa_roi_rdm_withcollapse  Compute ROI RDM and collapse across runs (1-3,2-4).
%
% USAGE:
...
%
% Inputs:
%   root, subj, roi, distType, session  - same as in previous script
...
%   *_collapsed1324.mat -> vec2save (flattened off-diagonals then diagonal)
%
% Notes:
%  - Folding uses the same method and runPair indices as your function
%    collapseRdm1324_roi.m (runPairsToUse = [2,5] for run pairs 1-3 & 2-4).
...
if strcmp(roi, 'WhBr_surf_r10_v100')
    glm = 'design_307';
else
    glm = 'design_307_MNI';
end
...
runPairsToUse = [2,5];          % indices selecting runPairs for 1-3 and 2-4 (matches your function)
...
collapsedFile = fullfile(outDirPerROI, sprintf('%s_ROI_RDM_%s_collapsed1324.mat', roi, distType));
save(collapsedFile, 'vec2save', 'nElem', 'nRun', 'runPairsToUse', 'created_date');
fprintf('Saved collapsed ROI RDM (1-3 & 2-4) to: %s\n', collapsedFile);
